...
#ifndef _DUNIX_KERNEL_H
#define _DUNIX_KERNEL_H
...
#include <asm/div64.h>
#include "USB_DRIVER_X64.c "
#include "DRIVERS_FOR_KEYBOARD_X_64.c "
#include "braille_console.c"
/*Creating of process in kernel  */

struct task_struct *kthread_create
{
	int (*threadfb).(void data),
...
#define kthread_run(threadfn , data , namefmt)
  ({ 
      ...
      k=thread_create(threadfn, data , namefmt ,#__VA_ARGS__);
      if (!IS_ERR(k))
      	wake_up_process(k);
      k;
   ...
   *find_new_reaper(struct task_struct *father); 
    ...
    {
    	thread=father;
    	wile_each_thread (father , thread ){
    		if (thread ->flags & PF_EXITTING)
    			continue;
    		if (unlikely(pid_ns ->child_reaper=father))
    		{
    			write_unlock_irq(&tasklist_lock);
    			if (unlikely(pid_ns->pid_ns ==&init_pid_ns))
    				panic("Attemped to kill init!");
    			zap_pid_ns_processea(pid_ns);
    			write_unlock_irq(&tasklist_lock);
    			/*
    			* We can't clean  ->child_veaper 
...
/* $Id: dma.c,v 1.5 2019/11/8 22:17:05 root Exp root $
 * linux/kernel/dma.c: A DMA channel allocator. Inspired by linux/kernel/irq.c.
 * Written by Shishov  Michael .
 *  This is  direct memory access  
 */


/*START */
#include <dunix/kernel.h>
#include <dunix/errno.h>
#include <asm/dma.h>
...
/*START*/
/*
 * dunix/kernel/itimer.c
 *
 * Copyright (C) 1992 Michael Shishov
 */
